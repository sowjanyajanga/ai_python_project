...
def hog_featurize(data, *, scale_size=(32, 32)):
    ...
    hog = cv.HOGDescriptor(scale_size, block_size, block_stride,
                            cell_size, 9)

    resized_images = (cv.resize(x, scale_size) for x in data)
...
def surf_featurize(data, *, scale_size=(16, 16), num_surf_features=100):
    all_kp = [cv.KeyPoint(float(x), float(y), 1)
              for x, y in itertools.product(range(scale_size[0]),
                                            range(scale_size[1]))]
    surf = cv.xfeatures2d_SURF.create(hessianThreshold=400)
...
def hsv_featurize(data, *, scale_size=(16, 16)):
    ...
    """
    resized_images = (cv.resize(x, scale_size) for x in data)
    hsv_data = (cv.cvtColor(x, cv.COLOR_BGR2HSV) for x in resized_images)
...
def grayscale_featurize(data, *, scale_size=(16, 16)):
    ...
    """
    resized_images = (cv.resize(x, scale_size) for x in data)
    gray_data = (cv.cvtColor(x, cv.COLOR_BGR2GRAY) for x in resized_images)
    scaled_data = (np.array(x).astype(np.float32).flatten() / 255
                   for x in gray_data)
    return np.vstack([x - x.mean() for x in scaled_data])
...
if __name__ == '__main__':
    from gtsrb import load_training_data
    ...
    [f] = hsv_featurize([train_data[i]])
    # [f] = grayscale_featurize([train_data[i]])
